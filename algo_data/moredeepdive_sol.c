...
void main(void){
	...
	//	short a[4]={10,20,30,40}; // &a[0]는 0x1000번지이다.
	//short *p=a;
	//short *q=a+3;
	//
	//	p++; q--;
...
void draw_pixel(int i, int j, int value, int(*p)[3])
{
	//(*p)[i*3+j] = value;
	p[i][j] = value;
}
...
{
	// p, q, r 선언
	int(*p)(int, int); void(*q)(void); int * (*r)(void);
...
{
	return func1()[p()]; // fa[1] 의 타입을 따져야 한다.
}
